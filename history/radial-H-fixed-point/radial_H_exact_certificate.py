...
TARGET = F(941, 40_000)             # per-radian target 0.023525
AREA_TARGET = F(739, 10_000)         # requested improved total bound 0.0739
M0, H = F(9999, 10_000), F(2957, 20_000)  # 0.9999, 0.14785
S_TEXT = ["0.9999", "1.47501648", "2.05059068", "2.75723440",
          "3.64393848", "4.86253053", "7.10984987"]
A_TEXT = ["0.36294599", "0.90963379", "1.38479070", "1.96023628",
          "2.66629690", "3.55037801", "4.75757582", "7.10984987"]
...
def low_enclosure() -> IV:
    m = 1-M0
    alpha = 2*asin_point(1/(2*M0))
    # atan(H/m)=pi/2-atan(m/H), avoiding a slow large-argument series.
    beta = pi_iv()/2 - atan_unit((1-M0)/H)
    ...
    k_h = alpha/(alpha + 2*asin_point(2*H))
...
def build_receipt() -> dict:
    ...
    # Direct total-area checks.  For high bins pi cancels exactly against
    # the projective-circle mass pi; only the low term needs pi's lower end.
    area_low_lb = low.lo * piv.lo
    area_high_lbs = [ints[j] / (S[j+1] if j < 6 else 2*S[6]) for j in range(7)]
    area_tail_lb = tail_int / (4*S[6])
    area_easy = H/2
    area_min_lb = min([area_easy, area_low_lb, *area_high_lbs, area_tail_lb])
    assert area_min_lb > AREA_TARGET
    ...
    return {
      ...
      "target_exact": fstr(TARGET), "target_decimal": "0.023525",
      "parameters": {"M0": fstr(M0), "H": fstr(H)},
      ...
      "total_area_target_exact": fstr(AREA_TARGET),
      "total_area_target_decimal": "0.0739",
      "easy_area_exact": fstr(area_easy),
      "low_total_area_lower": floor_decimal(area_low_lb),
      "high_total_area_lowers": [floor_decimal(x) for x in area_high_lbs],
      "tail_total_area_lower": floor_decimal(area_tail_lb),
      "minimum_total_area_proved_lower": floor_decimal(area_min_lb),
      "total_area_strict_margin_lower": floor_decimal(area_min_lb-AREA_TARGET),
...
def main() -> None:
    ...
    path=Path(__file__).with_name("radial_H_exact_certificate_receipt.json")
    if args.write:
    ...
    print("minimum total area proved lower =",receipt["minimum_total_area_proved_lower"])
    print("total area strict margin lower =",receipt["total_area_strict_margin_lower"])
