...
/-- The same derivative computation on the *full* admissible height interval
`[0,r)`.  This is the version needed when the target span is not bounded by
`strongEndpointPhi r strongA`: the comparison height `δ₀` is then only known to
lie below the radius. -/
...
`h ≤ δ₀` without any reference to the height cap `strongA`. -/
