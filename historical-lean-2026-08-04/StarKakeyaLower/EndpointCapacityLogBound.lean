...
The N=32 and N=64 frozen schedules use respectively three- and four-explicit-term
atanh expansions with rigorous rational tails.  Both proofs are in-kernel: they
differentiate the gap and prove the derivative nonnegative on `[0,1)`.
-/
...
/-- Exact three-term atanh/log upper bound used by the N=32 frozen schedule. -/
...
/-- Four explicit odd terms plus a geometric tail, as frozen by the N=64
certificate's `log_terms = 4` contract. -/
...
/-- Exact four-term atanh/log upper bound used by the frozen N=64 schedule. -/
