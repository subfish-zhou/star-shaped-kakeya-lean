...
The parameter-free front section of this file (the containment/mass interface,
the aggregate first-arc comparison, the raw family, the origin disks and the two
radial integral theorems) has been *moved* into
`StarKakeyaLower.CaseIRadialCore`, which imports no frozen witness constants.
It is re-exported by the import above, so every declaration that used to live
here is still available to every client of this module under its original name.
What remains below is the Li-specific conditional material, which genuinely
depends on `StarKakeyaLower.StrongerKernel`.
-/
