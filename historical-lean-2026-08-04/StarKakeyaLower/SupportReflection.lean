...
The three orientation-reversal identities `supportPoint_add_pi_neg`,
`supportNeedleTriangle_add_pi_neg` and `projectiveDirection_add_pi` were moved
out of this file into `StarKakeyaLower.UniversalNeedleAdapter`, which is CLEAN;
they are the prerequisites of `DirectionNeedle.orientPositive` and had to become
reachable without importing a frozen module.  They are re-exported by the import
above, so every client of this file still finds them under their original names.
-/
