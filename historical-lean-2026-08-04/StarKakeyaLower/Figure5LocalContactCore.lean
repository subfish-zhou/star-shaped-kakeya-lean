...
/-!
# Figure 5 core: the local-contact endpoint comparison
...
Every statement below is parameterised by a clean `Figure5EndpointDomain`, and
the module is CLEAN.  The historical `StarKakeyaLower.Figure5LocalContact` is a
thin frozen facade over it: it declares the original `Figure5SelectedGapGeometry`
inductive and the original `Figure5ActualSelectedTargetRelation` /
`Figure5LocalContactCertificate` structures at the `100π/5599` tuple, converts
them to and from the generic ones here, and re-exports every name below through
its import.  No geometry is duplicated: the facade owns no proof.
-/
...
theorem Figure5SelectedChangedLift.upper_selectedFor
    {r t a R₁ : ℝ} {q : DirectedSupportParameter}
    {D : FirstArcData (supportNeedleTriangle q.1 q.2.1 q.2.2) r}
    {A : NormalizedPolarArc}
    (H : Figure5SelectedChangedLift r t q D A)
    (hq : Figure5PositiveNormalizedSupport r a R₁ q) :
...
theorem Figure5SelectedChangedLift.lower_selectedFor
    {r t a R₁ : ℝ} {q : DirectedSupportParameter}
    {D : FirstArcData (supportNeedleTriangle q.1 q.2.1 q.2.2) r}
    {A : NormalizedPolarArc}
    (H : Figure5SelectedChangedLift r t q D A)
...
theorem figure5_local_contact_pointwiseFor
    {t gamma gval : ℝ} {A : NormalizedPolarArc}
    (_ht : 0 ≤ t) (hA : Icc A.lo A.hi ⊆ Icc 0 t)
    (hu : 2 * figure5UpperAnalyticAngle gamma t A - t < gval * t)
    (hl : 2 * figure5LowerAnalyticAngle gamma t A - t < gval * t) :
    (gamma : ProjectiveDirection) ∈
      quotientCenteredDilate (gval) Real.pi (t / 2) (t / 2) := by
  have hloA := (hA ⟨le_rfl, A.le_endpoints⟩).1
  have hhiA := (hA ⟨A.le_endpoints, le_rfl⟩).2
  have hlow : t / 2 - gval * (t / 2) < gamma := by
    simp only [figure5LowerAnalyticAngle] at hl
    have hm : gval * (t / 2) = gval * t / 2 := by ring
    rw [hm]
    linarith
  have hupp : gamma < t / 2 + gval * (t / 2) := by
    change 2 * (gamma + t - A.hi) - t < gval * t at hu
    have hm : gval * (t / 2) = gval * t / 2 := by ring
    rw [hm]
    linarith
...
theorem figure5_selected_strict_presentation_endpoints_activeFor
    {r t a R₁ : ℝ} {q : DirectedSupportParameter}
    {D : FirstArcData (supportNeedleTriangle q.1 q.2.1 q.2.2) r}
    {A : NormalizedPolarArc}
    (hsupport : Figure5PositiveNormalizedSupport r a R₁ q)
...
inductive Figure5SelectedGapGeometryFor (P : Figure5EndpointDomain)
    (r t δ₀ α : ℝ)
    (q : DirectedSupportParameter) (A : NormalizedPolarArc) : Prop where
  | nearHalf
      (hr : r ∈ Icc P.a P.r0)
      (ht0 : 0 < t) (hhalf : α ≤ t) :
      Figure5SelectedGapGeometryFor P r t δ₀ α q A
  | height
      (hr : r ∈ Icc P.a P.r0)
      (hδ0 : 0 < δ₀) (hδr : δ₀ < r)
  ...
  | baseBase
      ...
      (hsint : 0 ≤ Real.sin t)
      (radius_constraint : Real.sin α ≤ P.R1 * Real.sin t)
...
theorem Figure5SelectedGapGeometryFor.toGapSource
    {P : Figure5EndpointDomain}
    {r t δ₀ α : ℝ} {q : DirectedSupportParameter} {A : NormalizedPolarArc}
    (G : Figure5SelectedGapGeometryFor P r t δ₀ α q A)
    (hheight : 0 < q.2.1) (hheightA : q.2.1 ≤ P.a)
    (hselected : Icc A.lo A.hi ⊆ Icc 0 t) :
    Figure5UpperGapSourceFor P r t δ₀ α q := by
  cases G with
...
structure Figure5LocalContactCertificateFor (P : Figure5EndpointDomain)
    (r t : ℝ) (d : ProjectiveDirection) where
  q : DirectedSupportParameter
  D : FirstArcData (supportNeedleTriangle q.1 q.2.1 q.2.2) r
  A : NormalizedPolarArc
  gamma : ℝ
  support : Figure5PositiveNormalizedSupport r P.a P.R1 q
  selectedLift : Figure5SelectedChangedLift r t q D A
  gamma_direction : (gamma : ProjectiveDirection) = d
  upper_gap : 2 * figure5UpperAnalyticAngle gamma t A - t < P.g r * t
  lower_gap : 2 * figure5LowerAnalyticAngle gamma t A - t < P.g r * t
...
theorem figure5_easy_gapGeometryFor {P : Figure5EndpointDomain}
    {r t δ₀ α : ℝ} {qq : DirectedSupportParameter} {AA : NormalizedPolarArc}
    (hr : r ∈ Icc P.a P.r0)
    (ht0 : 0 < t) (hup : α ≤ t) :
    Figure5SelectedGapGeometryFor P r t δ₀ α qq AA :=
  .nearHalf hr ht0 hup
...
height for the radius (`0 < δ₀ < r`); no cap by `P.a` is used. -/
theorem figure5_hard_mixed_gapGeometryFor {P : Figure5EndpointDomain}
    ...
    Figure5SelectedGapGeometryFor P r t δ₀ α qq AA := by
  have hrhalf : r < 1 / 2 := Figure5EndpointDomain.radius_lt_half hr
...
theorem figure5ActualSelectedTargetRelationFor_of_principal
    (P : Figure5EndpointDomain)
    {r t δ₀ : ℝ} {q : DirectedSupportParameter}
    {D : FirstArcData (supportNeedleTriangle q.1 q.2.1 q.2.2) r}
    {A : NormalizedPolarArc}
    (hsupport : Figure5PositiveNormalizedSupport r P.a P.R1 q)
    (hstrict : q.2.1 < r)
    (hselected : Figure5SelectedChangedLift r t q D A)
    (hr : r ∈ Icc P.a P.r0)
    (hδ0 : 0 < δ₀) (hδr : δ₀ < r)
    ...
    ∃ gamma : ℝ, (gamma : ProjectiveDirection) = (q.1 : ProjectiveDirection) ∧
      Figure5ActualSelectedTargetRelationFor P r t δ₀ gamma q D A := by
  classical
  have hpi := Real.pi_pos
  have hrpos : 0 < r := hsupport.radius_pos
  have hh0 : 0 < q.2.1 := hsupport.height_pos
  have hhA : q.2.1 ≤ P.a := hsupport.height_le_a
  have hhr : q.2.1 ≤ r := hsupport.height_le_radius
  have hR₁ : 0 < P.R1 := P.R1_pos
  ...
  have hchordBase : ∀ φp φm : ℝ, 0 < Real.sin φp → 0 < Real.sin φm →
    ...
    linarith [hid]
  rcases figure5_selected_strict_presentation_endpoints_activeFor hsupport hstrict
  ...
  · -- the left strict presentation was selected
    ...
    · -- upper endpoint: the analytic lift is `t - (hi - q.angle)`
      ...
      rw [hup]
      exact figure5_easy_gapGeometryFor hr ht0 (by linarith)
    · -- lower endpoint: the analytic lift is `t + (lo - q.angle)`
      ...
      · -- selected mixed component: the centre sign is forced by selection
        ...
        have hloEq : Real.arctan (q.2.1 / (q.2.2 + 1 / 2)) = D.firstArc.lo - q.1 :=
          figure5_arctan_eq_of_base hb hlo0 hlolt hplus
        refine figure5_hard_mixed_gapGeometryFor (h := q.2.1) (base := q.2.2 + 1 / 2)
      ...
      · -- base-base component: both endpoints are needle-end tangencies
        ...
        have hnorm : q.2.1 ≤ P.R1 * Real.sin (D.firstArc.lo - q.1) :=
        ...
        have hraw : Figure5BaseRayRawProvenance t (figure5LowerSupport A q) := by
          ...
          · simp only [figure5LowerSupport, figure5LowerRotation, hAlo]
            push_cast; ring
        exact figure5_hard_baseBase_gapGeometryFor (ψ₂ := D.firstArc.hi - q.1)
  ...
  · -- the right strict presentation was selected
    ...
    · -- upper endpoint: the analytic lift is `t + (π - (hi - q.angle))`
      ...
      · -- selected mixed component on the right side
        ...
        refine figure5_hard_mixed_gapGeometryFor (h := q.2.1) (base := 1 / 2 - q.2.2)
          (spanA := A.span) hr hδ0 hδr ht ht0.le hrpos hh0 hstrict (by linarith)
          ?_ ?_ ?_
      ...
      · -- base-base component on the right side
        ...
        have hraw : Figure5BaseRayRawProvenance t (figure5UpperSupport t A q) := by
          ...
          · simp only [figure5UpperSupport, figure5UpperRotation, hAhi]
            push_cast; ring
        exact figure5_hard_baseBase_gapGeometryFor
    ...
    · -- lower endpoint: the analytic lift is `t - (π - (lo - q.angle))`
      have hlow : figure5LowerAnalyticAngle
        ...
        simp only [figure5LowerAnalyticAngle, hAlo]; ring
      rw [hlow]
      refine figure5_easy_gapGeometryFor hr ht0 ?_
      have : 0 ≤ Real.pi - (D.firstArc.lo - q.1) := by linarith
      linarith

noncomputable def figure5LocalContactCertificateFor_of_actualSelected
    ...
    (ht0 : 0 < t) (htop : t < Real.pi / 2 - Real.arctan (2 * r))
    (hsint : 0 ≤ Real.sin t) :
    Figure5LocalContactCertificateFor P r t (q.1 : ProjectiveDirection) := by
  let hex := figure5ActualSelectedTargetRelationFor_of_principal P hsupport hstrict
  ...
  exact
    { q := q
      ...
      upper_gap := hu.gap_lt HP
      lower_gap := hl.gap_lt HP }
