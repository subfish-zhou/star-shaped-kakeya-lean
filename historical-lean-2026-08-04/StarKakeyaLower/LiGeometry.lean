...
attained least/greatest endpoints; the former vacuous API asking for extrema
of the periodic trace on all of `ℝ` has been removed.  The abstract and
presentation-driven `ClosedTraceDecomposition` APIs cover empty, one, and two
...
The strict principal-window constructor consumes the exact circle-coverage
equality; deriving that equality globally from periodic support coordinates,
and then Li's two-component/exterior contact equations and ratio estimate,
remain.  No balance equation or endpoint-envelope identification is assumed
here.
-/
