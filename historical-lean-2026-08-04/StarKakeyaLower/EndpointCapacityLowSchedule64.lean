...
/-- The concrete 64 low atoms: one initial interval and 63 half-open bands. -/
def lowClass {E : Set Plane} (K : StarShapedKakeya E) (j : Fin 64) :
...
theorem b_strictMono : StrictMono b := by
  ...
  case «1» =>
    change (27912419 / 1000000000 : ℝ) < 34377677 / 1000000000
    norm_num
...
private theorem lowClass_mem_iff {E : Set Plane} (K : StarShapedKakeya E)
    (j : Fin 64) (theta : ProjectiveDirection) :
    theta ∈ lowClass K j ↔
      (if h : j.1 = 0 then
...
private theorem lowClass_disjoint_of_lt {E : Set Plane} (K : StarShapedKakeya E)
    {i j : Fin 64} (hlt : i < j) : Disjoint (lowClass K i) (lowClass K j) := by
  rw [Set.disjoint_left]
  intro theta hi hj
  rw [lowClass_mem_iff] at hi hj
  have hj0 : j.1 ≠ 0 := by omega
  simp only [hj0, ↓reduceDIte] at hj
  have hpred : i ≤ (⟨j.1 - 1, by omega⟩ : Fin 64) := by
...
/-- The 64 low atoms are pairwise disjoint, without measurability assumptions. -/
theorem lowClass_pairwiseDisjoint {E : Set Plane} (K : StarShapedKakeya E) :
    (Set.univ : Set (Fin 64)).PairwiseDisjoint (lowClass K) := by
  intro i _ j _ hij
...
/-- Exact exhaustion of the low nearest-radius region by the 64 atoms. -/
theorem iUnion_lowClass {E : Set Plane} (K : StarShapedKakeya E) :
    (⋃ j : Fin 64, lowClass K j) =
  ...
  · have hj : j = 0 := Fin.ext hj0
    ...
    simpa [lowClass, classLower] using
      endpointCapacityInitialClass_subset_windowDirections K
        (b_pos (0 : Fin 64)).le hbr (by simpa [classLower] using hbudget)
...
theorem lowEligible_subset_windowDirections {E : Set Plane}
    (K : StarShapedKakeya E) (i : Fin 64) (r : ℝ) :
    lowEligible K i r ⊆
...
def lowAtom {E : Set Plane} (K : StarShapedKakeya E) (j : ℕ) :
    Set ProjectiveDirection :=
  if h : j < 64 then lowClass K ⟨j, h⟩ else ∅

private theorem lowAtom_pairwiseDisjoint {E : Set Plane}
    (K : StarShapedKakeya E) :
    (Finset.range 64 : Set ℕ).PairwiseDisjoint (lowAtom K) := by
  intro i hi j hj hij
  have hi64 := Finset.mem_range.1 hi
  have hj64 := Finset.mem_range.1 hj
  change Disjoint (lowAtom K i) (lowAtom K j)
  unfold lowAtom
  rw [dif_pos hi64, dif_pos hj64]
...
private theorem iUnion_lowAtom_range {E : Set Plane}
    (K : StarShapedKakeya E) :
    (⋃ j ∈ Finset.range 64, lowAtom K j) = ⋃ j : Fin 64, lowClass K j := by
  ext theta
  simp only [mem_iUnion]
  constructor
  · rintro ⟨j, hj, htheta⟩
    have hj64 := Finset.mem_range.1 hj
    exact ⟨⟨j, hj64⟩, by simpa [lowAtom, hj64] using htheta⟩
  · rintro ⟨j, htheta⟩
...
theorem finiteLowLedger_to_lintegral
    ...
    (d : κ → NNReal) (c : NNReal)
    (hS : ∀ k ∈ I, S k ⊆ Finset.range 64)
    (hcover : ∀ j ∈ Finset.range 64,
    ...
    (c : ENNReal) * volume (⋃ j : Fin 64, lowClass K j) ≤
      ∫⁻ r in Icc (b 0) (switch 64), ENNReal.ofReal r *
        (radialSuperlevelOuter
          (directionTriangleUnion (universalPositiveNeedle K) Set.univ)).leftLim r := by
  rw [← iUnion_lowAtom_range K]
  exact (measure_biUnion_atoms_le_weighted_classes volume (lowAtom K) 64
...
theorem finiteLowLedger_to_outer
  ...
  calc
    _ ≤ ∑ k ∈ I, (d k : ENNReal) * volume (⋃ j ∈ S k, lowAtom K j) :=
      measure_biUnion_atoms_le_weighted_classes volume (lowAtom K) 64 I S d c
        hS (lowAtom_pairwiseDisjoint K) hcover (by simp)
    _ ≤ ∫⁻ r in Icc (b 0) (switch 64), ENNReal.ofReal r *
          (radialSuperlevelOuter
            (directionTriangleUnion (universalPositiveNeedle K) Set.univ)).leftLim r :=
      hweighted
    _ ≤ _ := K.selectedTriangleUnion_leftLim_lintegral_le (b_pos 0)

/-- The concrete coefficient used by the 3527/50000 witness. -/
def commonNN : NNReal := ⟨common, by norm_num [common]⟩

/-- Concrete low aggregate theorem.  The only remaining 64-row schedule
