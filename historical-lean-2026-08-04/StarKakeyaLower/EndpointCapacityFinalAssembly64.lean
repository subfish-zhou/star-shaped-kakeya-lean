...
`3527/50000` arithmetic.  The conditional top-level theorem isolates the two
...
def lowRadial : Set ℝ := Icc (b 0) (switch 64)
  ...
  | some k =>
      ...
      have hb0 : b 0 ≤ highB k := by
        ...
        have hk0 : highB 0 ≤ highB k := by
          ...
          norm_num
          positivity
        have hbSwitch : b 0 ≤ switch 64 := by
          change (931611 / 50000000 : ℝ) ≤ 129862023 / 250000000
          norm_num
...
theorem low_high_direction_cover {E : Set Plane} (K : StarShapedKakeya E) :
    (Set.univ : Set ProjectiveDirection) ⊆
      (⋃ j : Fin 64, lowClass K j) ∪ (⋃ k : ℕ, highClass K k) := by
  intro d _hd
...
theorem common_mul_projectiveVolume_le_selectedOuter_of_local_bounds
  ...
  have hmass : volume (Set.univ : Set ProjectiveDirection) ≤
    ...
    calc
      ...
      _ ≤ volume (⋃ j : Fin 64, lowClass K j) +
...
theorem final_3527_over_50000_of_concrete_local_bounds
  ...
  intro E K
  rw [show (3527 / 50000 : ℝ) = target by norm_num [target]]
...
/-- Universal lower bound at the schedule's actual implicit coefficient
`common * π`.  Unlike the strict rational corollary above, this theorem does
not round the headline constant downward. -/
theorem final_common_mul_pi_of_concrete_local_bounds
    (hlocal : ∀ {E : Set Plane} (K : StarShapedKakeya E),
      (∀ d, (universalPositiveNeedle K d).height < eta) →
      (ENNReal.ofReal common * volume (⋃ j : Fin 64, lowClass K j) ≤
        ∫⁻ u in Icc (b 0) (switch 64),
      ...
      (ENNReal.ofReal common * ∑' k, volume (highClass K k) ≤
        ∫⁻ u in ⋃ k, highLedger k,
          ENNReal.ofReal u * (selectedRadialQ K).leftLim u)) :
    ∀ (E : Set Plane), StarShapedKakeya E →
      ENNReal.ofReal (common * Real.pi) ≤ volume.toOuterMeasure E := by
  intro E K
  by_cases hheight : ∃ theta : Direction,
      eta ≤ (K.needleFamily theta).height K.center
  · exact (ENNReal.ofReal_le_ofReal common_mul_pi_lt_eta_half.le).trans
      (K.high_branch_outerMeasure hheight)
  · have hposHeight : ∀ d, (universalPositiveNeedle K d).height < eta :=
      fun d => universalPositiveNeedle_height_lt_of_no_high hheight d
    rcases hlocal K hposHeight with ⟨hlow, hhigh⟩
    have hselected :=
      common_mul_projectiveVolume_le_selectedOuter_of_local_bounds K hlow hhigh
    rw [show volume (Set.univ : Set ProjectiveDirection) =
        ENNReal.ofReal Real.pi by
      simp [ProjectiveDirection, AddCircle.measure_univ],
      ← ENNReal.ofReal_mul (by norm_num [common])] at hselected
...
/-- Universal lower bound at the exact minimum rational four-term surrogate,
with the projective-direction volume `π` left intact. -/
theorem final_certifiedCoefficient_mul_pi_of_concrete_local_bounds
    (hlocal : ∀ {E : Set Plane} (K : StarShapedKakeya E),
      (∀ d, (universalPositiveNeedle K d).height < eta) →
  ...
  by_cases hheight : ∃ theta : Direction,
      eta ≤ (K.needleFamily theta).height K.center
  · exact (ENNReal.ofReal_le_ofReal certifiedCoefficient_mul_pi_lt_eta_half.le).trans
      (K.high_branch_outerMeasure hheight)
  · have hposHeight : ∀ d, (universalPositiveNeedle K d).height < eta :=
      fun d => universalPositiveNeedle_height_lt_of_no_high hheight d
    rcases hlocal K hposHeight with ⟨hlow, hhigh⟩
    have hselected : ENNReal.ofReal certifiedCoefficient *
          volume (Set.univ : Set ProjectiveDirection) ≤
        volume.toOuterMeasure
          (directionTriangleUnion (universalPositiveNeedle K) Set.univ) := by
      have hmass : volume (Set.univ : Set ProjectiveDirection) ≤
        ...
        calc
          ...
          _ ≤ volume (⋃ j : Fin 64, lowClass K j) +
            ...
            add_le_add_right (measure_iUnion_le (fun k => highClass K k)) _
      calc
        ENNReal.ofReal certifiedCoefficient * volume Set.univ ≤
            ENNReal.ofReal certifiedCoefficient *
              (volume (⋃ j : Fin 64, lowClass K j) +
                ∑' k, volume (highClass K k)) := mul_le_mul_right hmass _
        _ = ENNReal.ofReal certifiedCoefficient *
              volume (⋃ j : Fin 64, lowClass K j) +
            ENNReal.ofReal certifiedCoefficient *
              ∑' k, volume (highClass K k) := mul_add _ _ _
        _ ≤ (∫⁻ u in Icc (b 0) (switch 64),
                ENNReal.ofReal u * (selectedRadialQ K).leftLim u) +
              (∫⁻ u in ⋃ k, highLedger k,
                ENNReal.ofReal u * (selectedRadialQ K).leftLim u) :=
    ...
    rw [show volume (Set.univ : Set ProjectiveDirection) =
        ENNReal.ofReal Real.pi by
      simp [ProjectiveDirection, AddCircle.measure_univ],
      ← ENNReal.ofReal_mul (by norm_num [certifiedCoefficient])] at hselected
    exact hselected.trans <| by
      calc
        volume.toOuterMeasure
            (directionTriangleUnion (universalPositiveNeedle K) Set.univ) ≤
