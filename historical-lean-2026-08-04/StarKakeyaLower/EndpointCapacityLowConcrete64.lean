...
/-- 64 head pieces followed by the 63 shared switch pieces. -/
abbrev LowRow := Fin 64 ⊕ Fin 63
...
def lowRowHi : LowRow → ℝ
  | .inl m => if h : m.1 < 63 then b ⟨m.1 + 1, by omega⟩ else switch 1
  | .inr q => switch ⟨q.1 + 2, by omega⟩

def lowRowWindow : LowRow → Fin 64
  | .inl _ => 0
  | .inr q => ⟨q.1 + 1, by omega⟩

def lowRowAtoms : LowRow → Finset ℕ
  | .inl m => Finset.range (m.1 + 1)
  | .inr q => (Finset.range 64).filter (fun j => q.1 + 1 ≤ j)

private theorem lowRowAtoms_sub (k : LowRow) : lowRowAtoms k ⊆ Finset.range 64 := by
  intro j hj
  rcases k with m | q
...
private theorem classLower_nonneg (j : Fin 64) : 0 ≤ classLower j := by
  by_cases hj : j.1 = 0
  · simp [classLower, hj]
...
-- Exhaustively checks the frozen endpoint bounds for all 127 concrete rows.
...
private theorem eta_le_switch_succ (q : Fin 63) :
...
private theorem lowRowAtoms_eligible {E : Set Plane} (K : StarShapedKakeya E)
  ...
  have hj64 := Finset.mem_range.1 (lowRowAtoms_sub k hjS)
  rw [lowEligible]
  apply mem_iUnion.2
  refine ⟨⟨j, hj64⟩, ?_⟩
  apply mem_iUnion.2
  ...
  · refine ⟨?_, ?_⟩
    · rcases k with m | q
      · have hjle : j ≤ m.1 := by simpa [lowRowAtoms] using hjS
        have hb : b ⟨j, hj64⟩ ≤ b m := b_mono (Fin.mk_le_mk.mpr hjle)
        exact hb.trans hr.1.le
      · exact (b_le_eta ⟨j, hj64⟩).trans ((eta_le_switch_succ q).trans hr.1.le)
    · rcases k with m | q
      · simp only [lowRowWindow]
        norm_num [outerRadius]
        exact sq_nonneg (classLower ⟨j, hj64⟩)
      · have hij : (⟨q.1 + 1, by omega⟩ : Fin 64) ≤ (⟨j, hj64⟩ : Fin 64) :=
        ...
        calc
          ...
          _ ≤ 1 / 4 + classLower ⟨j, hj64⟩ ^ 2 := by
            have hm := classLower_mono hij
            have h0 := classLower_nonneg (⟨q.1 + 1, by omega⟩ : Fin 64)
            nlinarith
          _ = classLower ⟨j, hj64⟩ ^ 2 + 1 / 4 := by ring
  · simpa [lowAtom, hj64] using hjtheta
...
private theorem switch_strictMono : StrictMono switch := by
  ...
  case «1» =>
    change (84298807 / 250000000 : ℝ) < 340207097 / 1000000000
    norm_num

private theorem switch_one_le_succ (q : Fin 63) :
    switch 1 ≤ switch ⟨q.1 + 1, by omega⟩ :=
  switch_strictMono.monotone (Fin.mk_le_mk.mpr
    (Nat.succ_le_succ (Nat.zero_le q.1)))

private theorem switch_le_last (i : Fin 65) : switch i ≤ switch 64 :=
...
private theorem lowRows_weighted_le_lintegral {E : Set Plane}
  ...
  calc
    ...
    _ ≤ ∫⁻ r in Icc (b 0) (switch 64), f r := by
...
private def lowHeadPiece (m : ℕ) : ℝ :=
  if hm : m < 64 then
    endpointWindowPayment (outerRadius 0) (b ⟨m, hm⟩)
      (if hlast : m < 63 then b ⟨m + 1, by omega⟩ else switch 1)
  else 0

/-- The head rows telescope from any cut `n` to the common endpoint `switch 1`.
The downward induction is structural; in particular, no enumeration of the 64
possible cuts is involved. -/
private theorem lowHead_filtered_telescope (n : ℕ) (hn : n < 64) :
    ∑ m ∈ Finset.Icc n 63, lowHeadPiece m =
      endpointWindowPayment (outerRadius 0) (b ⟨n, hn⟩) (switch 1) := by
  apply Nat.decreasingInduction (n := 63) (motive := fun k _ =>
    ∑ m ∈ Finset.Icc k 63, lowHeadPiece m =
      endpointWindowPayment (outerRadius 0) (b ⟨k, by omega⟩) (switch 1))
  · intro k hk ih
    have hk64 : k < 64 := by omega
    have hsets : Finset.Icc k 63 = insert k (Finset.Icc (k + 1) 63) := by
      ext m
      simp
      omega
    rw [hsets, Finset.sum_insert]
    · rw [ih]
      rw [lowHeadPiece, dif_pos hk64, dif_pos hk]
...
private theorem lowHead_filtered_eq (j : Fin 64) :
    (∑ m : Fin 64, if j.1 ∈ lowRowAtoms (.inl m) then
        (lowRowWeight (.inl m) : ℝ) else 0) = lowHeadPayment j := by
  rw [← Finset.sum_filter]
  calc
    _ = ∑ m ∈ Finset.Icc j.1 63, lowHeadPiece m := by
      ...
      · intro m hm
        ...
        change endpointWindowPayment (outerRadius 0) (b m)
          (if h : m.1 < 63 then b ⟨m.1 + 1, by omega⟩ else switch 1) =
            lowHeadPiece m.1
...
`lowPayment`. -/
private theorem lowShared_filtered_eq (j : Fin 64) :
    (∑ q : Fin 63, if j.1 ∈ lowRowAtoms (.inr q) then
        (lowRowWeight (.inr q) : ℝ) else 0) =
      ∑ i : Fin 64, if 1 ≤ i.1 ∧ i.1 ≤ j.1 then lowSegmentPayment i else 0 := by
  rw [← Finset.sum_filter, ← Finset.sum_filter]
  apply Finset.sum_bij (fun q _ => (⟨q.1 + 1, by omega⟩ : Fin 64))
  · intro q hq
...
private theorem lowRow_cover_eq (j : Fin 64) :
...
private theorem lowRow_cover (j : ℕ) (hj : j ∈ Finset.range 64) :
    commonNN ≤ ∑ k ∈ (Finset.univ : Finset LowRow).filter
      (fun k => j ∈ lowRowAtoms k), lowRowWeight k := by
  let jf : Fin 64 := ⟨j, Finset.mem_range.1 hj⟩
...
/-- Fully concrete low ledger: the 127 radial rows have no remaining schedule
parameters or analytic hypotheses. -/
theorem common_mul_lowUnion_le_lintegral {E : Set Plane}
    (K : StarShapedKakeya E) :
    ENNReal.ofReal common * volume (⋃ j : Fin 64, lowClass K j) ≤
      ∫⁻ r in Icc (b 0) (switch 64), ENNReal.ofReal r *
        (radialSuperlevelOuter
...
/-- The exact minimum rational surrogate, without the shorter `common`
rounding used by the original certificate headline. -/
def certifiedCoefficientNN : NNReal :=
  ⟨certifiedCoefficient, by norm_num [certifiedCoefficient]⟩

private theorem certifiedCoefficient_lowRow_cover
    (j : ℕ) (hj : j ∈ Finset.range 64) :
    certifiedCoefficientNN ≤ ∑ k ∈ (Finset.univ : Finset LowRow).filter
      (fun k => j ∈ lowRowAtoms k), lowRowWeight k := by
  let jf : Fin 64 := ⟨j, Finset.mem_range.1 hj⟩
  have h : certifiedCoefficient ≤
      ∑ k ∈ (Finset.univ : Finset LowRow).filter
        (fun k => j ∈ lowRowAtoms k), (lowRowWeight k : ℝ) := by
    rw [show j = jf.1 by rfl, lowRow_cover_eq jf]
    exact certifiedCoefficient_le_lowPayment jf
  exact_mod_cast h

theorem certifiedCoefficient_mul_lowUnion_le_lintegral
    {E : Set Plane} (K : StarShapedKakeya E) :
    ENNReal.ofReal certifiedCoefficient * volume (⋃ j : Fin 64, lowClass K j) ≤
      ∫⁻ r in Icc (b 0) (switch 64), ENNReal.ofReal r *
        (radialSuperlevelOuter
          (directionTriangleUnion (universalPositiveNeedle K) Set.univ)).leftLim r := by
  rw [ENNReal.ofReal_eq_coe_nnreal (by norm_num [certifiedCoefficient])]
  exact finiteLowLedger_to_lintegral K Finset.univ lowRowAtoms lowRowWeight
    certifiedCoefficientNN (fun k _ => lowRowAtoms_sub k)
    certifiedCoefficient_lowRow_cover (lowRows_weighted_le_lintegral K)
