...
* **orientation** – an arbitrary raw `DirectionNeedle` has a *signed* support
  ...
  filled triangle and the pair of base-endpoint norms.  That normalization and
  its four basic lemmas were moved into the CLEAN
  `StarKakeyaLower.UniversalNeedleAdapter`, which this file imports; they are
  generic and must be usable without the frozen witness constants.
